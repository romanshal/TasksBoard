...
namespace TasksBoard.Application.Features.BoardNotices.Queries.GetPaginatedBoardNoticesByBoardId
{
    public class GetPaginatedBoardNoticesByBoardIdQueryHandler(
        ...
        IMapper mapper,
        IUserProfileHandler profileHandler) : IRequestHandler<GetPaginatedBoardNoticesByBoardIdQuery, Result<PaginatedList<BoardNoticeDto>>>
    {
        ...
        public async Task<Result<PaginatedList<BoardNoticeDto>>> Handle(GetPaginatedBoardNoticesByBoardIdQuery request, CancellationToken cancellationToken)
        {
            ...
            var count = await _unitOfWork.GetBoardNoticeRepository().CountByBoardIdAsync(BoardId.Of(request.BoardId), cancellationToken);
            if (count == 0)
            {
            ...
            var boardNotices = await _unitOfWork.GetBoardNoticeRepository().GetPaginatedByBoardIdAsync(BoardId.Of(request.BoardId), request.PageIndex, request.PageSize, cancellationToken);
