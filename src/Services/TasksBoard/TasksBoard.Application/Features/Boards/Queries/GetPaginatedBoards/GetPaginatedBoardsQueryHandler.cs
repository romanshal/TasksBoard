...
namespace TasksBoard.Application.Features.Boards.Queries.GetPaginatedBoards
{
    public class GetPaginatedBoardsQueryHandler(
        ...
        IMapper mapper) : IRequestHandler<GetPaginatedBoardsQuery, PaginatedList<BoardDto>>
    {
        private readonly IUnitOfWork _unitOfWork = unitOfWork;
        private readonly ILogger<GetBoardsQueryHandler> _logger = logger;
        private readonly IMapper _mapper = mapper;

        public async Task<PaginatedList<BoardDto>> Handle(GetPaginatedBoardsQuery request, CancellationToken cancellationToken)
        {
